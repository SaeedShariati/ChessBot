...
def save_batch(tensors, values, policies, prefix, batch_num):
    ...
    # Save
    filename = f"./puzzle_data/{prefix}_batch_{batch_num+181:04d}.pt"
    torch.save({
        'X': X,
        'y_value': y_value,
        'y_policy': y_policy,
    ...
    return filename
def process_puzzle_csv(zst_path, max_positions=500000, batch_size=50000, output_prefix="chess_puzzle"):
    print(f"\nProcessing {zst_path}...")
